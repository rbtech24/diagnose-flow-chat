...
const formatUserData = async (userId?: string | null) => {
  ...
  try {
    ...
    console.error(`Error formatting user data for ${userId}:`, err);
    return {
      id: userId,
      name: 'Unknown User',
      email: '',
...
export const fetchCommunityPostById = async (id: string): Promise<CommunityPost> => {
  try {
    ...
    console.error("Error in fetchCommunityPostById:", error);
    throw error;
  }
};
...
export const markCommentAsAnswer = async (postId: string, commentId: string): Promise<void> => {
  try {
    ...
    console.error("Error in markCommentAsAnswer:", error);
    throw error;
  }
};
